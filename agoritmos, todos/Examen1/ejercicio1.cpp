...
public:
    ...
    Patient(string n, string i, int e, int t) {
        name = n;
        id = i;
        emergencyLevel = e;
        estimatedTime = t;
...
public:
    ...
    void insertPatient(string name, string id, int level, int time) {
        Patient* newPatient = new Patient(name, id, level, time);
    ...
    void servePatient() {
        ...
        cout << "\nServing: " << served->patient->name << " (ID: " << served->patient->id << ")\n";
    ...
    while (current) {
        ...
        cout << "| Name: " << setw(12) << left << current->patient->name
             << "| ID: " << setw(6) << left << current->patient->id
...
int main() {
    ...
    do {
        ...
        if (option == 1) {
            ...
            cin >> time;
            cin.ignore();
            system.insertPatient(name, id, level, time);
        } else if (option == 2) {
            system.servePatient();
        }
