...
void ActualizarLista() {
    ...
    queue<Estudiante> copia = cola;
    while (!copia.empty()) {
        Estudiante e = copia.front();
        copia.pop();
        SendMessage(hLista, LB_ADDSTRING, 0, (LPARAM)(e.nombre + " - " + e.correo).c_str());
    }
}
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR args, int nCmdShow) {
    ...
    HWND hwnd = CreateWindow("RegistroWin", "Sistema de Registro Academico",
                             WS_OVERLAPPEDWINDOW | WS_VISIBLE,
                             100, 100, 470, 420, NULL, NULL, NULL, NULL);
    CrearControles(hwnd);
